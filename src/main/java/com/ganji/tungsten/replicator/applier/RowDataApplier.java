...
//	abstract protected boolean filterSchemaTable(String schema, String table);
    ...
    private String ColValue2String(ColumnSpec cspec, ColumnVal cvalue) {
        ...
        if( cvalue.getValue() == null )
        	return null;
        logger.debug( "type=" + String.valueOf(cspec.getType()) +
        		",length=" + String.valueOf(cspec.getLength()) +
        		",value class=" + cvalue.getValue().getClass().toString());
...
//        else if( cspec.getType() == Types.INTEGER && cspec.getLength() == 8 ) {
//        	
//        }
        else {
            if( cvalue.getValue() != null ) 
